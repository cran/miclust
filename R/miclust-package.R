...
#'   integrate the results of multiply imputed datasets while accounting for
#'   the uncertainty that the imputations introduce in the final results. See
#'   `Procedure' below for further details on how the tool works.
#' @section Procedure: The tool consists of a two-step procedure. In the first
...
#'   datasets. In the latter case, \code{getdata} needs to by used first to get
#'   data prepared. In the second step, the \code{miclust} performs k-means
#'   clustering with selection of the final number of clusters and an optional
#'   (backward or forward) variable selection procedure. Specific \code{summary}
#'   and \code{plot} methods are provided to summarize and visualize the impact
